...
export default function Upload() {
  ...
  async function handleUpload() {
    ...
    try {
      ...
      if (!data.ok) {
        ...
        setMsg({ type: "warning", text: "Upload completed but 0 references detected. Check that the Excel file contains a 'Repository Number' column." });
      } else {
        setMsg({ type: "success", text: `Upload successful — ${data.count} SDS references imported.` });
        setFile(null);
      }
    } catch {
      setMsg({ type: "error", text: "Server error while uploading. Please try again." });
    } finally {
      setLoading(false);
    }
  }
  ...
  const sdsColumns = [
    "Repository Number", "Business Entity", "Chemical Product",
    "Manufacturer's Name", "Revision Date", "Verification Date",
  ];
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 16 }}>
          ...
          <div style={fieldWrap}>
            ...
            <input
              ...
              style={inputStyle}
            />
            <span style={hint}>Stored as uppercase with underscores</span>
          </div>
      ...
      <div style={{ ...card, marginTop: 24 }}>
        <div style={cardHeader}>
          ...
          <span style={{
            ...
            {sdsColumns.length} columns
          </span>
        </div>
        <p style={{ color: "#64748b", fontSize: 13, margin: "0 0 14px" }}>
          Your Excel file must include these column headers. <strong>Repository Number</strong> is the primary key — rows without it are skipped.
        </p>
        <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
          {sdsColumns.map((col, i) => (
            <div key={col} style={{
